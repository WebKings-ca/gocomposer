...
class PluginTest extends CommandTestBase
{
    ...
    {
        $process = new Process('composer global config repositories.gocomposer path $(pwd)', $this->sandbox);
        $process->run();
        $process = new Process('composer global require grasmash/gocomposer "*@dev"', $this->sandbox);
        $process->run();
